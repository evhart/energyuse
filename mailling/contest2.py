...
with open('./mails/participants.csv','rU') as csvfile:
   ...
   for user in users:
        #print(user)
        msg_plain = render_to_string('contest2.txt', {'username': user['firstname']})
        #print(msg_plain)
        messages.append(('Contribute to EnergyUse and win an Amazon voucher!', msg_plain, 'Energyuse.eu <contact@energyuse.eu>', [user['email']]))
    
   print len(messages)
   
   for user in users2:
        #print(user)
        msg_plain = render_to_string('contest2.txt', {'username': user['username']})
        #print(msg_plain)
        messages.append(('Contribute to EnergyUse and win an Amazon voucher!', msg_plain, 'Energyuse.eu <contact@energyuse.eu>', [user['email']]))
