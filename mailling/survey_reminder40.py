...
with open('./mails/participants.csv','rU') as csvfile:
   ...
   for user in users2:
        ...
        if user['email'] not in []:
            msg_plain = render_to_string('survey2.txt', {'username': user['username']})
            #print(msg_plain)
            messages.append(('Reminder: Your feedback for the Energy Trial is really important to us.', msg_plain, 'Energyuse.eu <contact@energyuse.eu>', [user['email']]))
