...
with open('./mails/participants.csv','rU') as csvfile:
    ...
    for user in users:
        print(user)
        msg_plain = render_to_string('halloween.txt', {'username': user['firstname']})
        #print(msg_plain)
        messages.append(('Discover where your zombies are and win an Amazon voucher!', msg_plain, 'Energyuse.eu <contact@energyuse.eu>', [user['email']]))
    
    
    for user in users2:
        print(user)
        msg_plain = render_to_string('halloween.txt', {'username': user['username']})
        #print(msg_plain)
        messages.append(('Discover where your zombies are and win an Amazon voucher!', msg_plain, 'Energyuse.eu <contact@energyuse.eu>', [user['email']]))
